...
app.get('/dashboard', (c) => {
  return c.html(`
    ...
    <div class="mb-6">
        <h3 class="text-sm font-bold text-cyan-500/70 mb-3 px-1 uppercase tracking-widest">Select Station</h3>
        <div id="radio-station-tabs" class="flex gap-2 overflow-x-auto pb-4 border-b border-cyan-900/30">
            </div>
    </div>
    ...
    <div class="mt-16 p-6 rounded-3xl bg-purple-900/10 border border-purple-500/20 relative overflow-hidden">
        <div class="absolute top-0 right-0 p-8 opacity-10 text-6xl">
            <i class="fas fa-quote-right text-purple-400"></i>
        </div>
        
        <div class="relative z-10">
            <h3 class="text-2xl font-black neon-text mb-2 flex items-center gap-3">
                <i class="fas fa-pen-nib text-purple-400"></i>
                라디오 신청 사연 저장소
            </h3>
            <p class="text-gray-400 text-sm mb-8 px-1">원하는 사연을 복사하여 라디오 게시판에 활용하세요.</p>
    ...
    try {
        ...
        tabContainer.innerHTML = uniqueStations.map(station => `
            <button onclick="filterRadioByStation('${station}')" 
                    class="station-tab-btn px-4 py-2 rounded-xl font-bold transition-all text-gray-400 border border-cyan-800/30 hover:border-cyan-500/50"
        ...
        console.error('라디오 데이터 로드 실패:', err);
    }
}

// 탭을 눌렀을 때 해당 방송사의 카드를 그려주는 함수
function filterRadioByStation(stationName) {
    const radioList = document.getElementById('radio-list');
    
    // 버튼 활성화 스타일 적용
    document.querySelectorAll('.station-tab-btn').forEach(btn => {
        btn.classList.toggle('tab-active', btn.getAttribute('data-station') === stationName);
    });
    ...
    radioList.innerHTML = filtered.map(item => `
        <div class="card p-5 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/50 transition-all group">
            ...
            <h4 class="text-lg font-black text-white mb-2 group-hover:text-cyan-400 transition-colors">${item.title}</h4>
            <p class="text-xs text-gray-400 mb-6 line-clamp-2">${item.description || '플레이브 곡을 신청해 주세요!'}</p>
            <div class="flex gap-2">
                <a href="${item.link}" target="_blank" class="flex-1 text-center py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs font-bold transition-all">
                    신청하러 가기
                </a>
            </div>
        </div>
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(() => {
        alert('📋 신청 문구가 복사되었습니다!');
    });
}

// 페이지 로드 시 실행
document.addEventListener('DOMContentLoaded', renderRadioSection);
        </script>
    </body>
    </html>
  `)
